...
cp "$BIN" "$APP/Contents/MacOS/ErDanPet"
cp Info.plist "$APP/Contents/Info.plist"
HASH=$(security find-identity -p codesigning 2>/dev/null | grep "$CERT" | head -1 | awk '{print $2}')
codesign --force --sign "${HASH:--}" --identifier io.erdan.pet "$APP" 2>/dev/null
open "$APP"                              # 用 open 启动:授权归属才是"二蛋"自己,而不是启动它的终端
sleep 1
if pgrep -x ErDanPet >/dev/null; then echo "二蛋已启动(ErDanPet.app,已签名 $( [ -n "$HASH" ] && echo 自签 || echo adhoc ))"; else echo "⚠️ 启动失败,看 Console.app 里 ErDanPet 的日志"; fi
