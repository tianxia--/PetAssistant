...
- **聊天大脑**:接你自己的 LLM(OpenAI / Anthropic 接口格式),带多轮上下文,回复支持 Markdown 格式化显示;也可以直接借用本机已登录的命令行(`claude` / `pi`),不用填地址、不用填 Key、不额外花钱
...
1. **配模型**:填你的 LLM 接口(地址 / 模型 ID / API Key)。没配也能用,消息会先记进收件箱。
   - 也可以在格式下拉里选**「本机 Claude / pi(已登录)」**:二蛋直接调用该命令行,复用你已经登录好的会话,地址和 Key 两栏会自动隐藏。不读取任何凭证文件,就是把命令行原样跑起来。代价是每次回复要冷启动一个进程(约 4–8 秒)。
